...
/*
 * Do not pass floating-point values through LVGL's printf formatter here.
 * Float formatting support varies with the LVGL/newlib build configuration;
 * formatting a fixed-point integer is smaller and works reliably everywhere.
 */
                             ...
                             const char *title, const char *unit)
{
    ...
    /*
     * Keep the value area at a stable size.  LV_SIZE_CONTENT on this nested
     * flex container can briefly collapse while a label's text is replaced,
     * which clips the newly drawn digits while leaving part of the unit visible.
     */
...
void ui_init(void)
{
    ...
    lv_obj_center(root);
    lv_obj_set_style_pad_all(root, 5, LV_PART_MAIN);
    ...
    lv_obj_set_style_text_color(title, lv_palette_main(LV_PALETTE_ORANGE), LV_PART_MAIN);
    lv_obj_set_style_text_font(title, &lv_font_montserrat_14, LV_PART_MAIN);
    lv_obj_set_style_pad_ver(title, 1, LV_PART_MAIN);

    label_temp = create_card(root, LV_PALETTE_RED,   "Temperature", "°C");
...
void ui_set_temperature(float temp) {
    if (label_temp) set_label_fixed_1(label_temp, temp);
}
