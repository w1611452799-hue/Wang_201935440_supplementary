...
static esp_err_t i2c_read_byte(i2c_master_dev_handle_t dev, uint8_t reg_addr, uint8_t *data) {
    return i2c_master_transmit_receive(dev, &reg_addr, 1, data, 1, -1);
}

static esp_err_t i2c_read_bytes(i2c_master_dev_handle_t dev, uint8_t reg_addr, size_t data_len, uint8_t *data) {
    return i2c_master_transmit_receive(dev, &reg_addr, 1, data, data_len, -1);
}

static esp_err_t i2c_write_byte(i2c_master_dev_handle_t dev, uint8_t reg_addr, uint8_t data) {
    uint8_t buffer[2] = {reg_addr, data};
    return i2c_master_transmit(dev, buffer, 2, -1);
}

bme280_handle_t bme280_create(i2c_master_dev_handle_t dev_handle)
{
    bme280_dev_t *sens = (bme280_dev_t *) calloc(1, sizeof(bme280_dev_t));
    if (!sens) return NULL;
    sens->i2c_dev = dev_handle;
...
static esp_err_t bme280_read_uint16(bme280_handle_t sensor, uint8_t addr, uint16_t *data)
{
    ...
    bme280_dev_t *sens = (bme280_dev_t *) sensor;
    uint8_t data0, data1;
    if (i2c_read_byte(sens->i2c_dev, addr, &data0) != ESP_OK) {
        return ret;
    }
    if (i2c_read_byte(sens->i2c_dev, addr + 1, &data1) != ESP_OK) {
        return ret;
    }
    *data = (data0 << 8) | data1;
    return ESP_OK;
...
bool bme280_is_reading_calibration(bme280_handle_t sensor)
{
    uint8_t rstatus = 0;
    bme280_dev_t *sens = (bme280_dev_t *) sensor;
    if (i2c_read_byte(sens->i2c_dev, BME280_REGISTER_STATUS, &rstatus) != ESP_OK) {
        return false;
    }
...
esp_err_t bme280_read_coefficients(bme280_handle_t sensor)
{
    ...
    if (i2c_read_byte(sens->i2c_dev, BME280_REGISTER_DIG_H1, &data) != ESP_OK) {
        return ESP_FAIL;
    }
    ...
    if (i2c_read_byte(sens->i2c_dev, BME280_REGISTER_DIG_H3, &data) != ESP_OK) {
        return ESP_FAIL;
    }
    sens->data_t.dig_h3 = data;
    if (i2c_read_byte(sens->i2c_dev, BME280_REGISTER_DIG_H4, &data) != ESP_OK) {
        return ESP_FAIL;
    }
    if (i2c_read_byte(sens->i2c_dev, BME280_REGISTER_DIG_H4 + 1, &data1) != ESP_OK) {
        return ESP_FAIL;
    }
    sens->data_t.dig_h4 = (data << 4) | (data1 & 0xF);
    if (i2c_read_byte(sens->i2c_dev, BME280_REGISTER_DIG_H5 + 1, &data) != ESP_OK) {
        return ESP_FAIL;
    }
    if (i2c_read_byte(sens->i2c_dev, BME280_REGISTER_DIG_H5, &data1) != ESP_OK) {
        return ESP_FAIL;
    }
    sens->data_t.dig_h5 = (data << 4) | (data1 >> 4);
    if (i2c_read_byte(sens->i2c_dev, BME280_REGISTER_DIG_H6, &data) != ESP_OK) {
        return ESP_FAIL;
    }
...
esp_err_t bme280_set_sampling(bme280_handle_t sensor, bme280_sensor_mode mode, bme280_sensor_sampling tempSampling, bme280_sensor_sampling pressSampling, bme280_sensor_sampling humSampling, bme280_sensor_filter filter, bme280_standby_duration duration)
{
    ...
    if (i2c_write_byte(sens->i2c_dev, BME280_REGISTER_CONTROLHUMID, bme280_getctrl_hum(sensor)) != ESP_OK) {
        return ESP_FAIL;
    }
    if (i2c_write_byte(sens->i2c_dev, BME280_REGISTER_CONFIG, bme280_getconfig(sensor)) != ESP_OK) {
        return ESP_FAIL;
    }
    if (i2c_write_byte(sens->i2c_dev, BME280_REGISTER_CONTROL, bme280_getctrl_meas(sensor)) != ESP_OK) {
        return ESP_FAIL;
    }
    return ESP_OK;
}
...
esp_err_t bme280_default_init(bme280_handle_t sensor)
{
    ...
    uint8_t chipid = 0;
    bme280_dev_t *sens = (bme280_dev_t *) sensor;
    if (i2c_read_byte(sens->i2c_dev, BME280_REGISTER_CHIPID, &chipid) != ESP_OK) {
    ...
    if (i2c_write_byte(sens->i2c_dev, BME280_REGISTER_SOFTRESET, 0xB6) != ESP_OK) {
        return ESP_FAIL;
    }
...
esp_err_t bme280_take_forced_measurement(bme280_handle_t sensor)
{
    ...
    if (sens->ctrl_meas_t.mode == BME280_MODE_FORCED) {
        ...
        if (i2c_read_byte(sens->i2c_dev, BME280_REGISTER_STATUS, &data) != ESP_OK) {
            return ESP_FAIL;
        }
        while (data & 0x08) {
            i2c_read_byte(sens->i2c_dev, BME280_REGISTER_STATUS, &data);
...
esp_err_t bme280_read_temperature(bme280_handle_t sensor, float *temperature)
{
    ...
    uint8_t data[3] = { 0 };
    bme280_dev_t *sens = (bme280_dev_t *) sensor;
    if (i2c_read_bytes(sens->i2c_dev, BME280_REGISTER_TEMPDATA, 3, data) != ESP_OK) {
        return ESP_FAIL;
    }
...
esp_err_t bme280_read_pressure(bme280_handle_t sensor, float *pressure)
{
    ...
    if (i2c_read_bytes(sens->i2c_dev, BME280_REGISTER_PRESSUREDATA, 3, data) != ESP_OK) {
        return ESP_FAIL;
    }
